...
# ... (Configuration and imports remain same)
...
# Update paths to use node_{i}.csv
trainers = [NodeTrainer(i, os.path.join(DATA_DIR, f'node_{i}.csv'), NODE_MODEL_PATHS[i-1]) for i in NODES]
...
def fl_job():
    ...
    if any_trained:
        ...
        for i, trainer in enumerate(trainers):
            ...
            node_model = SimpleNN()
            try:
                node_model.load_state_dict(torch.load(NODE_MODEL_PATHS[i]))
        ...
        print("No nodes had enough new data to train.")
